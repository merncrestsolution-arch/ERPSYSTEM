...
export default function Vehicles() {
  ...
  const openEditModal = (vehicle: Vehicle) => {
    ...
    setLicenseExpiry(vehicle.revenue_license_expiry || '');
    setModalOpen(true);
  };
  ...
  const handleSaveVehicle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (window.electronAPI) {
        if (editingId) {
          // @ts-ignore
          await window.electronAPI.updateVehicle(editingId, payload);
        } else {
          // @ts-ignore
          await window.electronAPI.addVehicle(payload);
        }
        await loadVehicles();
        setModalOpen(false);
      }
              ...
              {vehicles.filter(v => v.registration_number.toLowerCase().includes(search.toLowerCase()) || v.driver_name?.toLowerCase().includes(search.toLowerCase())).map((v) => (
                <tr key={v.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                    <button onClick={() => openEditModal(v)} className="text-slate-400 hover:text-blue-600 transition-colors mr-3"><Edit2 size={18} /></button>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center sticky top-0 bg-white z-10">
              <h3 className="text-lg font-bold text-slate-800">{editingId ? 'Edit Vehicle' : 'Add Vehicle'}</h3>
              <button onClick={() => setModalOpen(false)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            <form onSubmit={handleSaveVehicle} className="p-4 sm:p-6 space-y-4">
              <div>
